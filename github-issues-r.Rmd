...
m1 <- map(snapp_wg_scicomp_issues_proto, "labels") %>% flatten() %>% map_chr_hack(., "name") %>% as.data.frame()

m1_graph <- ggplot(m1, aes(x = .)) +
  geom_bar()
...
snapp_wg_scicomp_issues <- gh("/repos/SNAPP/snapp-wg-scicomp/issues", 
  ...
  data.frame(number = map_int(., "number"), #Calls the issue number
             ...
             title = map_chr(., "title"), #Calls the title of the issue
             labels1 = map_df(., ~.$"labels"[1]), # gives everything under the first label
             labels2 = map_df(., ~.$"labels"[2]) # inaccurate currently if the issue does not have a second label it will go to the next issue with a second label and repeat until finished
             )
  } %>% 
...
g <- ggplot(snapp_wg_scicomp_issues_filtered, aes(x = labels1.name)) +
  geom_bar()
