...
snapp_wg_scicomp_issues_proto[[2]]$labels %>% map(., c('[[', "name")) #returns null values
...
snapp_wg_scicomp_issues <- gh("/repos/SNAPP/snapp-wg-scicomp/issues", 
  ...
  data.frame(number = map_int(., "number"), #Calls the issue number
             ...
             title = map_chr(., "title"), #Calls the title of the issue
             labels = map(., "labels") %>% stringr::str_flatten(";")# Currently returning a list of everything in the labels list for the entire issues list 
