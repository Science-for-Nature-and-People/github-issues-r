...
map_dfr_hack <- function(.x, .f, ..., .id = NULL) {
  map(.x, .f, ...,) %>%
    map_if(is.null, ~ NA_character_) %>%
    flatten_dfr()
}
...
gh_issues_private_df <- gh_issues_private %>%
  {
    data.frame(labels = map_dfr(., ~ .$"labels"[1])) %>% select("labels.name")
  }

str(gh_issues_private)                        
...
snapp_wg_scicomp_issues <- gh("/repos/SNAPP/snapp-wg-scicomp/issues", 
  ...
  data.frame(number = map_int(., "number"), #Calls the issue number
             ...
             title = map_chr(., "title"), #Calls the title of the issue
             labels1 = map_dfr(., ~.$"labels"[1]), # gives everything under the first label
             labels2 = map_dfr(., ~.$"labels"[2]) # inaccurate currently if the issue does not have a second label it will go to the next issue with a second label and repeat until finished
             )
  } %>% 
...
count(snapp_wg_scicomp_issues_filtered, snapp_wg_scicomp_issues_filtered$labels1.name)
