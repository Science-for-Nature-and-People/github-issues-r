...
snapp_wg_scicomp_issues <- gh("/repos/SNAPP/snapp-wg-scicomp/issues", 
  ...
  data.frame(number = map_int(., "number"), #Calls the issue number
             ...
             labels = map_chr_hack(., c("labels", "{name}")) #This will return the label the issue has been assigned Currently not working properly because there is a level between labels and name that I am having trouble calling Right now it only retuns NA
             )
  } 
