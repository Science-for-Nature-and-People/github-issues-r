...
modify_depth_hack <- function(.x, .depth, .f, ...){
  modify_depth(.x, .depth, .f, ...) %>%
    modify_if(is.null, ~ NA_character_)
}

m1 <- map(snapp_wg_scicomp_issues_proto, "labels")

#m2 <- map_chr_hack(snapp_wg_scicomp_issues_proto, "labels")

m3 <- snapp_wg_scicomp_issues_proto[[1]]$labels[[1]]$name



m4 <- for(i in 1:length(snapp_wg_scicomp_issues_proto)) {
  map(snapp_wg_scicomp_issues_proto, "labels")
}
```
...
snapp_wg_scicomp_issues <- gh("/repos/SNAPP/snapp-wg-scicomp/issues", 
  ...
  data.frame(number = map_int(., "number"), #Calls the issue number
             ...
             title = map_chr(., "title"), #Calls the title of the issue
             labels = map(., "labels") %>% stringr::str_flatten(";")
             #This will return the label the issue has been assigned Currently not working properly because there is a level between labels and name that I am having trouble calling Right now it only retuns NA
